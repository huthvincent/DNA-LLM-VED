...
Evo2_score <- read_tsv("Annotation/Evo2_6Models_Inference_result.6475639.UCSC_Fullanno.LCR_addMAF_Tier.Jan19.txt.gz")
...
run_corr_table_all <- function(df, model_name) {
  ...
  stopifnot("phastCons100way" %in% names(df))
  stopifnot("MAF_tier" %in% names(df))
  
  safe_spearman <- function(dat) {
    dat <- dat[is.finite(dat[[delta_col]]) & is.finite(dat$phastCons100way), , drop = FALSE]
    if (nrow(dat) <= 2) return(list(n = nrow(dat), rho = NA_real_, p = NA_real_))
    ct <- cor.test(abs(dat[[delta_col]]), dat$phastCons100way,
  ...
  out <- out[, c("model","Category","N_variants","rho","P")]
  out
}
...
write.table(
  all_tables,
  "Evo2_spearman_correlation_summary_AbsScore_MAFbin_ALL.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
