...
data_clean=data_clean[!data_clean$category == "Intergenic",]

table(data_clean$ExonicFunc.ensGene)
...
data_clean$group <- as.factor(data_clean$ExonicFunc.ensGene)
...
dat <- data_clean %>%
  mutate(
    ...
    cat   = as.factor(ExonicFunc.ensGene)
  ) %>%
