setwd("G:\\Project\\Evo2VEP\\Allele_age\\2d")
...
# -----------------------------
Evo2_score <- read_tsv(
  "Evo2_6Models_Inference_result.SDS.4326095.Allele_Match_FlipEvo2.UCSC_Fullanno.LCR.July12_SDS_flip_Final.txt.gz",
...
bootstrap_spearman_fixedFrac <- function(df, model_name,
  ...
  stopifnot("SDS_Final" %in% names(df))
  ...
  # Pre-filter
  df2 <- df[
    is.finite(get(delta_col)) & is.finite(SDS_Final) & !is.na(get(af_col))
  ]
