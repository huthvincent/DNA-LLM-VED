...
HAR <- fread("HAR_hg38_bed_region.txt", header = FALSE)
setnames(HAR, c("CHR", "start", "end"))
regions_dt <- as.data.table(HAR)
...
data.table::fwrite(
  res_sitesub$summary,
  file =
    "HAR_raw_SiteSubsample_80pctWithinChrxAF_2000fold_OverAllSummary.tsv",
  sep = "\t",
  quote = FALSE,
  na = "NA"
)
...
data.table::fwrite(
  tbl_boot,
  file =
    "HAR_raw_SiteSubsample_80pctWithinChrxAF_2000fold.tsv",
  sep = "\t",
  quote = FALSE,
  na = "NA"
)
...
data.table::fwrite(
  ci_tbl,
  file =
    "HAR_raw_SiteSubsample_80pctWithinChrxAF_2000fold.summary.tsv",
  sep = "\t",
  quote = FALSE,
  na = "NA"
)
